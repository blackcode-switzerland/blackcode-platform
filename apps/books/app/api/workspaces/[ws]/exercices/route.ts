  ...
  if (slug) {
    ...
    if (!e) {
      throw Errors.notFound(
        ...
        "`bk books entity list` shows this workspace's books and their slugs",
      );
    }
    entityId = e.id;
  }
...
export const POST = apiHandler(async (req: NextRequest, { params }: Params) => {
  ...
  if (!entity) {
    throw Errors.notFound(
      "entity_not_found",
      `no book with slug "${slug}"`,
      "`bk books entity list` shows the slugs; `bk books entity create` makes a new book",
    );
  }

  const row = await createExercice(ctx.workspace.id, {
    entityId: entity.id,
    year,
  });
