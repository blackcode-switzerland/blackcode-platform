...
// reasons, neither of which is "b/billing has tokens of its own":
// `bk login --server <billing>` authorizes at this origin, and the page that
// revokes a token has to be able to reach one from the origin it is served on.
//
// It is also how the first external customer's system gets its credential:
// `docs/billing-app-plan/integration-surface.md` §7 has their administrator mint
// one here in a browser for a service account. Minting is session-only, so that
// step cannot be scripted — which is the intended shape, not a limitation.
//
