...
/**
 * The one entity this app defines, so that every layer below has something
 ...
 * (`bc:books:acme/note/7`); the serial `id` is an internal detail that no
 * surface should ever print. `apps/issues` learned this the hard way: `bk trash`
 * exposed row ids until Phase 8.
 */
export const notes = booksSchema.table('notes', {
  id: serial('id').primaryKey(),
  workspace_id: integer('workspace_id')
    .notNull()
    .references(() => booksWorkspaces.id, { onDelete: 'cascade' }),
  /** Workspace-scoped #number. Allocated from `books.note_counters` below. */
 ...
 */
export const noteCounters = booksSchema.table('note_counters', {
  workspace_id: integer('workspace_id')
    .primaryKey()
    .references(() => booksWorkspaces.id, { onDelete: 'cascade' }),
