...
| Version source | [`cli-version.ts`](../../packages/platform-agent/src/cli-version.ts) `CLI_PACKAGE` (whose npm dist-tags `latest`/`min` are read live), the `FALLBACK_*` constants, and the `BK_CLI_*` env names | `CLI_PACKAGE` → their npm package, fallbacks reset to `1.0.0`; their own dist-tags from then on |
