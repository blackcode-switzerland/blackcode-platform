...
export const GET = apiHandler(async (req: NextRequest, { params }: Params) => {
  ...
  if (!Number.isInteger(n) || n < 1) {
    throw Errors.badRequest(
      ...
      "try `bk books entry list` for the numbers",
    );
  }
  ...
  if (slug) {
    ...
    if (e.bookkeeping_regime === "simplified") {
      const [row] = await getDb()
        ...
        .where(
          and(
            eq(booksRiEntry.workspace_id, ctx.workspace.id),
            eq(booksRiEntry.entity_id, e.id),
            eq(booksRiEntry.seq, n),
      ...
      if (!row)
        throw Errors.notFound(
          "entry_not_found",
          `no entry #${n} in ${slug}'s recettes-dépenses journal`,
          "bk books entry list --entity shows the numbers",
        );
      return NextResponse.json(
        publicRiEntry(
          row,
          await journalScopeOf(row.entity_id, row.exercice_id),
        ),
      );
    }
  }
  ...
  if (found) {
    return NextResponse.json(
      publicEntry(
        found,
        await journalScopeOf(found.entry.entity_id, found.entry.exercice_id),
      ),
    );
  }
  ...
  if (riRow)
    return NextResponse.json(
      publicRiEntry(
        riRow,
        await journalScopeOf(riRow.entity_id, riRow.exercice_id),
      ),
    );

  throw Errors.notFound(
    "entry_not_found",
    `no entry #${n} in this workspace`,
    "`bk books entry list --entity <book>` shows the #numbers; note they are workspace-wide, not the journal no.",
  );
});
