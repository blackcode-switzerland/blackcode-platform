...
export async function getSourceBySeq(
  workspaceId: number,
  seq: number,
): Promise<BooksSource | null> {
  const [row] = await getDb()
    .select()
    .from(booksSource)
    .where(
      and(eq(booksSource.workspace_id, workspaceId), eq(booksSource.seq, seq)),
    )
    .limit(1);
  return row ?? null;
}
...
export async function runbookOf(
  sourceId: number,
): Promise<BooksRunbook | null> {
  const [row] = await getDb()
    .select()
    .from(booksRunbook)
...
export async function entitySlugsById(
  workspaceId: number,
...
export async function reconcileSource(
  ...
  const [pull] = await db
    ...
    .where(
      and(
        eq(booksSourcePull.source_id, source.id),
        sql`${booksSourcePull.closing_balance} IS NOT NULL`,
      ),
    )
  ...
  const openings = await db
    .select({
      account_no: booksOpeningBalance.account_no,
      amount: booksOpeningBalance.amount,
    })
    .from(booksOpeningBalance)
    .innerJoin(
      booksExercice,
      eq(booksExercice.id, booksOpeningBalance.exercice_id),
    )
    .where(
      and(
...
export function publicManifestRow(
  m: BooksDriveManifest,
    ...
    const [entity] = await tx
      ...
      .where(
        and(
          ...
          eq(booksEntity.slug, data.entitySlug),
        ),
      )
      .limit(1);
    if (!entity)
      throw new SourceRefused(
        "bad_entity",
        `no book with slug "${data.entitySlug}"`,
  ...
  if (data.importMapping !== undefined) {
    if (data.importMapping !== null) {
      ...
      if (bad)
        throw new SourceRefused(
          "bad_mapping",
          bad, // Points at --help, not at the guide topic: money-in shows the COMMAND but
          // no worked mapping, so the recovery it named did not deliver one. The
          // shape lives beside the flag that takes it. (Phase-8 cold run, 2026-08-20.)
          "`bk books source mapping-set --help` shows the whole shape, with a worked example",
        );
    }
    ...
    if (data.drawsFromSeq === null) patch.draws_from = null;
      ...
      const [parent] = await getDb()
        ...
        .where(
          and(
            eq(booksSource.workspace_id, workspaceId),
            eq(booksSource.seq, data.drawsFromSeq),
          ),
        )
        .limit(1);
      if (!parent) {
...
function mappingRefusal(m: DelimitedMapping): string | null {
  ...
  if (
    !m.columns ||
    typeof m.columns.date !== "string" ||
    typeof m.columns.label !== "string"
  ) {
  ...
  if (m.decimal !== "." && m.decimal !== ",")
    return 'decimal must be "." or ","';
  ...
  return db.transaction(async (tx) => {
    ...
    if (source.retired)
      throw new SourceRefused(
        ...
        "a retired source takes no new pulls",
      );
    ...
    const [existing] = await tx
      ...
      .where(
        and(
          eq(booksSourcePull.source_id, source.id),
          eq(booksSourcePull.file, data.file),
        ),
      )
      .limit(1);
 ...
 */
export async function setRunbook(
  workspaceId: number,
  sourceSeq: number,
  data: SetRunbookData,
): Promise<BooksRunbook> {
