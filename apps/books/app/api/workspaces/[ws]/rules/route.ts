...
import { listRules, createRule, publicRule, teachingSeqs } from '@/lib/db/queries/rules'
...
export const GET = apiHandler(async (req: NextRequest, { params }: Params) => {
  ...
  const seqs = await teachingSeqs(rows)
  return jsonList(rows.map((r) => publicRule(r, seqs.get(r.id) ?? null)), null)
})
...
export const POST = apiHandler(async (req: NextRequest, { params }: Params) => {
  ...
  return NextResponse.json(publicRule(row, null), { status: 201 })
})
