 ...
 * remembered.
 */
const OTHER_SCHEMAS = ['issues', 'sales', 'books', 'scaffold']
...
// ===========================================================================
// THE THIRD BOUNDARY: THIS APP'S TENANCY IS `billing.workspaces`
// ===========================================================================
// **Copied from `apps/sales/lib/app-isolation.test.ts` — the scaffold does NOT
// carry this case**, which is one of three gaps
// `docs/billing-app-plan/phase-0-register-the-app.md` §2 records. An app copied
// from the scaffold has two of the three boundaries and looks complete.
//
// It was added to sales on 2026-08-10 after the bug it catches had been live
// since Phase 2 and had survived four phases of verification green.
//
...
// `platform.workspaces`. Phase 2 moved that app's workspaces to
...
// tenants.
//
// ── IT IS ALREADY LOAD-BEARING IN THIS APP ─────────────────────────────────
// `app/dashboard/page.tsx` here is a SERVER COMPONENT that reads the database
// directly — no `fetch`, no route, no token. `lib/cli-parity.test.ts` cannot see
// it at all, which is CLAUDE.md's "a route is not a page" corollary. This is the
// only automated check standing between that page and another app's tenancy.
//
...
function appSourceFiles(root: string): string[] {
  ...
  const walk = (dir: string) => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      // banned list, so a scan including itself reports itself. It fired within
      // a minute of the namespace check being added to sales.
      //
...
describe("this app's tenancy is its own", () => {
  ...
  for (const file of files) {
    ...
    // Injected and watched the sales suite stay GREEN before this block existed,
  ...
  it('resolves workspaces and members through billing.*, never platform.*', () => {
    expect(
      offenders,
      "this app's workspaces are `billing.workspaces`. These helpers read " +
        '`platform.workspaces`, which is `apps/issues`\' data despite the name, so a page ' +
        'or route using one answers with ANOTHER APP\'S TENANTS. Use this app\'s own ' +
        '`lib/db/queries/workspaces.ts`.\n' +
        'The two boundaries above cannot see this: the import is legal and the file names ' +
        'no schema. See this block\'s header.'
    ).toEqual([])
  })
})
