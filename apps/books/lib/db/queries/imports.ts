...
export async function nextSeq(
  tx: Tx,
  workspaceId: number,
  entityType: keyof typeof SEQ_TABLES,
): Promise<number> {
...
export async function importStatement(
  ...
  if (problems.length > 0) {
    throw new ImportRefused(
      "does_not_reconcile",
      "the file does not reconcile against itself",
      problems,
    );
  }
  ...
  return db.transaction(async (tx) => {
    const [source] = await tx
      ...
      .where(
        and(
          ...
          eq(booksSource.seq, sourceSeq),
        ),
      )
      .limit(1);
    if (!source)
      throw new ImportRefused("source_not_found", `no source #${sourceSeq}`, [
        "bk books source list shows the numbers",
      ]);
    if (source.retired)
      throw new ImportRefused(
        "source_retired",
        `source #${sourceSeq} is retired`,
        ["a retired source takes no new files"],
      );
    if (source.entity_id === null) {
      throw new ImportRefused(
        "source_unattached",
        `source #${sourceSeq} belongs to no book`,
        ["attach it to a book first"],
      );
    }
    ...
    if (!entity)
      throw new ImportRefused(
        "source_unattached",
        `source #${sourceSeq}'s book is gone`,
      );
    const simplified = entity.bookkeeping_regime === "simplified";
    const journal = simplified
      ? ("recettes_depenses" as const)
      : ("grand_livre" as const);

    const ledgerAccount = (source.ledger_accounts ?? [])[0] ?? null;
    if (!simplified && !ledgerAccount) {
      throw new ImportRefused(
        "no_ledger_account",
        `source #${sourceSeq} names no ledger account`,
        [
    ...
    if (!simplified && source.draws_from !== null) {
      ...
      if (!parent) {
        throw new ImportRefused(
          "draws_from_missing",
          `source #${sourceSeq} settles into a source that no longer exists`,
          [
      ...
      if (parentAccount && parentAccount === ledgerAccount) {
        throw new ImportRefused(
          ...
          [
            ...
            `create one with \`bk books account create --entity <book> --no <no> --class 2 --label-fr <text> --position autres_dettes_ct\`, then \`bk books source edit ${sourceSeq} --ledger-account <no>\``,
          ],
        );
      }
    }
    ...
    for (const l of stmt.lines) {
      ...
      if (!x)
        yearProblems.push(
          `${l.ref}: booked ${l.booked}, and the book has no exercice ${l.booked.slice(0, 4)}`,
        );
      else if (x.status === "closed")
        yearProblems.push(
          `${l.ref}: booked ${l.booked} into CLOSED exercice ${x.year}`,
        );
    }
    if (yearProblems.length > 0) {
      throw new ImportRefused(
        "no_open_exercice",
        "lines fall outside an open exercice",
        [...yearProblems, "open the year first: bk books exercice create"],
      );
    }
    ...
    if (refs.length > 0) {
      if (simplified) {
        const rows = await tx
          ...
          .where(
            and(
              eq(booksRiEntry.source_id, source.id),
              inArray(booksRiEntry.bank_ref, refs),
            ),
          );
        ...
        const rows = await tx
          ...
          .where(
            and(
              eq(booksEntry.source_id, source.id),
              inArray(booksEntry.bank_ref, refs),
            ),
          );
        for (const r of rows) if (r.ref) existing.add(r.ref);
      }
    ...
    for (const line of stmt.lines) {
        ...
        matchesRule(
          {
            ...
            lines: [{ debit: line.amount, credit: 0 }],
          },
          r,
        ),
      );
      ...
      if (simplified) {
        ...
        const bankSide = {
          entry_id: e.id,
          account_no: ledgerAccount,
          position: 1,
        };
        const openSide = {
          entry_id: e.id,
          account_no: null as string | null,
          position: 2,
        };
...
export async function postEntry(
  ...
  return db.transaction(async (tx) => {
    const [entry] = await tx
      ...
      .where(
        and(
          eq(booksEntry.workspace_id, workspaceId),
          eq(booksEntry.seq, entrySeq),
    ...
    if (!entry) {
      throw new PostRefused(
        "entry_not_found",
        `no entry #${entrySeq}`,
        "bk books entry list shows the numbers — note that an RI journal has no posting lifecycle",
      );
    }
    ...
    if (v?.verdict === "blocked") {
      ...
      throw new PostRefused(
        ...
        resolves ??
          "resolve what the verdict flagged, then have the reviewer re-run; blocked entries do not post",
      );
    }
