...
export async function closeExercice(
  ...
  if (exercice.ends_on > today) {
    throw new CloseRefused(
      ...
      // The hint used to end "shorten the exercice first". Nothing can:
      // `bk books exercice` carries list, create and close, `create` takes only
      // --year, and no verb writes ends_on. Naming a recovery the CLI cannot
      // perform is the failure this hint exists to prevent, so it says what is
      // actually reachable and stops there. (A short exercice remains a real
      // statutory case with no write door — booksFrontend tracker, 2026-08-20.)
      "a close files the year as its final result and there is no reopen (art. 958f CO). For a figure before year end, read it instead: `bk books cr` for the result so far, or `bk books cr --by-month`",
    );
  }
  ...
  if (staged.length > 0) {
    ...
    throw new CloseRefused(
      ...
      `work the list to the end first: bk books worklist --entity <book>, then bk books entry post ${staged[0].seq}`,
    );
  }
  ...
  const [next] = await db
    ...
    .where(
      and(
        ...
        eq(booksExercice.year, exercice.year + 1),
      ),
    );
  if (!next) {
    throw new CloseRefused(
      ...
      // --entity is REQUIRED on that command; the hint printed a form that
      // cannot run. Found by the phase-8 cold run, 2026-08-20.
      `open it first: bk books exercice create --entity <book> --year ${exercice.year + 1}`,
    );
  }
  ...
  const openingMap = new Map(
    openings.map((o) => [o.account_no, toCentimes(o.amount)]),
  );
  ...
  if (!hasRetained) {
    throw new CloseRefused(
      ...
      `add it: bk books account create --entity <book> --no ${RETAINED_EARNINGS} --class 2 --label-fr "Bénéfice / perte reporté(e)" --position resultat_reporte`,
    );
  }
  carried.push({
    account_no: RETAINED_EARNINGS,
    amount: fromCentimes(retained),
  });
