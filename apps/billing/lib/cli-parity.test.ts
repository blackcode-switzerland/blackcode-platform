...
const EXCLUDED_PATHS = new Map<string, string>([
  ...
  // ── THE ACCOUNT SURFACE, MOUNTED IN PHASE 0 ───────────────────────────────
  // This app took fullstack ownership from its first commit and mounted the four
  // password routes and the CLI-authorize route that issues, sales and books all
  // have. Every entry below is the same entry, for the same reason, that those
  // three carry — copy them with the routes if you copy this app.
  //
  ...
  [
    ...
    'the browser half of `bk login` — the binary OPENS /cli/authorize in a browser and ' +
      ...
      'Mounted rather than skipped because `bk login --server https://billing…` is a ' +
...
const UNSERVED_OPERATIONS = new Map<string, string>([
  // NOTE: `POST /api/workspaces` is NOT here, unlike in the scaffold. This app
  // SERVES it (`bk billing workspace create`), because a workspace here is a
