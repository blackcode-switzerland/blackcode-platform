...
// The OTP password-reset flow, in this app's own styling. Two modes:
...
//
// ---------------------------------------------------------------------------
// WHY THE TWO MODES CALL DIFFERENT ROUTES
// ---------------------------------------------------------------------------
// Authenticated resets go through `lib/mutations.ts`' `useRequestPasswordOtp` /
// `useConfirmPassword` (`POST /api/me/password/request-otp` + `/confirm`, no
// email in the body — the session already names the account). The logged-out
// flow has no session to name it with, so it calls
// `POST /api/auth/password-reset/request` + `/confirm` directly with `email`;
// those are public routes, not covered by an account-scoped mutation hook, and
// `lib/mutations.ts` is WS F's file — this component stays the one place that
// knows both shapes rather than asking that file to grow an `email?` parameter
// only the logged-out half needs.
//
// Either way the rule that must not diverge — OTP length, expiry, attempt cap,
// which failure maps to which code — lives in `@blackcode/platform-auth` and
// the shared route factories, where `tsc` enforces it.
//
// ---------------------------------------------------------------------------
...
import { useRequestPasswordOtp, useConfirmPassword } from '@/lib/mutations'
...
export function PasswordResetFlow({ authenticated, presetEmail, onDone, onCancel }: Props) {
  ...
  async function requestCode(e?: React.FormEvent) {
    ...
    try {
      if (authenticated) {
        const j = await requestOtp.mutateAsync(undefined)
        setSentTo(j.email)
      } else {
        await call<{ ok: true }>('/api/auth/password-reset/request', {
          method: 'POST',
  ...
  async function confirmReset(e: React.FormEvent) {
    ...
    try {
      if (authenticated) {
        ...
        await call<{ ok: true }>('/api/auth/password-reset/confirm', {
          method: 'POST',
  ...
  return (
    <div className="space-y-4">
      {step === 'request' ? (
        <form onSubmit={requestCode} className="space-y-3" data-testid="reset-request-form">
        ...
        <form onSubmit={confirmReset} className="space-y-3" data-testid="reset-confirm-form">
          <p className="text-sm text-muted-foreground">
