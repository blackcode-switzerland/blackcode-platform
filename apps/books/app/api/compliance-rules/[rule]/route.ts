...
export const GET = apiHandler(async (_req: NextRequest, { params }: Params) => {
  ...
  if (!found) {
    throw Errors.notFound(
      "rule_not_found",
      `no compliance rule "${rule}"`,
      "`bk books compliance list` shows every rule id — they are words (bk-001, ret-002), not numbers",
    );
  }
  ...
  async (req: NextRequest, { params }: Params) => {
    ...
    if (!user)
      throw Errors.unauthorized(
        "a review records who signed it — authenticate first",
      );
    ...
    try {
      const row = await reviewComplianceRule(rule, {
        ...
        editedLogic:
          typeof body.edited_logic === "string" ? body.edited_logic : null,
      ...
      if (e instanceof ComplianceRefused) {
        // e.suggestion, not nothing: the refusal carries one and the 400 branch
        // below already passes it. Dropping it on the 404 made the most
        // recoverable failure the only one with no way out.
        if (e.code === "rule_not_found")
