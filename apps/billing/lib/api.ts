...
/**
 * THIS APP'S WORKSPACES ARE `billing.workspaces` (Phase 7, 2026-08-11).
 *
 ...
 * After the split, a billing workspace id written into it is read back by
 ...
 * `bk billing workspace use` persists its choice in the CLI's own per-app
 * config (agent 5 keyed `ActiveWorkspaces` by app slug for exactly this reason).
 */
const billingWorkspaceSource: WorkspaceSource = {
 ...
 *
 * ── WHY NOT A `billing.uploads` TABLE ──────────────────────────────────────
...
const noUploadLedger: UploadLedger = {
  attribute: async () => {
    throw new Error(
      'apps/billing records no uploads: it serves no /api/upload route. If you ' +
...
export const appContext: AppContext = {
  ...
  // Its own, since 2026-08-11. See `billingWorkspaceSource` above.
  //
  ...
  workspaces: billingWorkspaceSource,
  ...
  footprint: billingFootprintSource,
