...
export async function earliestExerciceYear(
  entityId: number,
): Promise<number | null> {
...
export async function setOpenings(
  ...
  if (earliest !== null && exercice.year !== earliest) {
    throw new OpeningRefused(
      ...
      `openings after the first year are produced by closing the year before: bk books exercice close --entity <book> --year ${exercice.year - 1}`,
    );
  }
  ...
  const db = getDb();
  const ghosts = await accountsNotInChart(
    db,
    entityId,
  ...
  const chart = new Map(
    (await listAccounts(entityId)).map((a) => [a.no, Number(a.class)]),
  );
  let actif = 0;
  let passif = 0;
  ...
  return {
    written,
    totalActif: actif.toFixed(2),
    totalPassif: passif.toFixed(2),
  };
}
