 ...
 *
 * To run migrations by hand:  npm run db:migrate --workspace=billing
 *
 ...
 */
/**
 * ── b/books SHIPPED WITHOUT THIS FILE, AND SO WOULD HAVE b/billing ─────────
 * Copied verbatim from apps/books/scripts/migrate-if-enabled.mjs on 2026-09-16
 * (only the two `--workspace=` strings differ). `apps/_scaffold` carries
 * NEITHER this script nor the `postbuild` line that runs it, while issues,
 * sales and books all do — so an app copied from the scaffold and deployed with
 * `RUN_MIGRATIONS=1` would match nothing, succeed, serve traffic, and apply
 * ZERO migrations against a database where its schema does not exist.
 *
 * Nothing in docs/adding-an-app.md catches it. The check is the DEPLOY LOG.
 */
...
if (!enabled) {
  console.log(
    '• postbuild: skipping migrations (RUN_MIGRATIONS is not set).\n' +
      ...
      '  Production sets RUN_MIGRATIONS=1. To migrate by hand: npm run db:migrate --workspace=billing'
  )
  process.exit(0)
}
