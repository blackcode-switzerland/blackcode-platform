// Invitations into one of this app's workspaces — `books.invitations`.
//
// ---------------------------------------------------------------------------
...
// `books.workspace_members`. Somebody invited here may already have an
...
import { booksInvitations, booksWorkspaceMembers, users } from '../schema'
...
export async function createInvitation(input: {
  ...
  const existingMember = await getDb()
    ...
    .where(
      and(eq(booksWorkspaceMembers.workspace_id, input.workspaceId), eq(users.email, email))
    )
    .limit(1)
...
export async function listInvitations(workspaceId: number): Promise<InvitationRow[]> {
  return await getDb()
    .select({
      id: booksInvitations.id,
      email: booksInvitations.email,
      role: booksInvitations.role,
      token: booksInvitations.token,
      status: booksInvitations.status,
      expires_at: booksInvitations.expires_at,
      created_at: booksInvitations.created_at,
      invited_by_name: users.name,
      invited_by_email: users.email,
    })
    .from(booksInvitations)
    .innerJoin(users, eq(users.id, booksInvitations.invited_by))
    .where(
      and(
        eq(booksInvitations.workspace_id, workspaceId),
        eq(booksInvitations.status, 'pending')
      )
    )
    .orderBy(desc(booksInvitations.created_at))
}
...
export async function revokeInvitation(workspaceId: number, id: number): Promise<boolean> {
  const rows = await getDb()
    .update(booksInvitations)
    .set({ status: 'revoked' })
    .where(
      and(
        eq(booksInvitations.id, id),
        eq(booksInvitations.workspace_id, workspaceId),
        eq(booksInvitations.status, 'pending')
      )
    )
    .returning({ id: booksInvitations.id })
...
export async function findUsableInvitation(token: string) {
  const rows = await getDb()
    .select()
    .from(booksInvitations)
    .where(eq(booksInvitations.token, token))
    .limit(1)
  const inv = rows[0]
  if (!inv) return null
...
export async function markAccepted(id: number, userId: number): Promise<void> {
  await getDb()
    ...
    .where(eq(booksInvitations.id, id))
}
