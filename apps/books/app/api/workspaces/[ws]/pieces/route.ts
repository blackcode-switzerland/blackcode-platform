...
import { booksEntry, booksPieceInbox } from '@/lib/db/schema'
...
export const GET = apiHandler(async (req: NextRequest, { params }: Params) => {
  ...
  // Resolve matched entries and duplicate pointers to workspace #numbers.
  ...
  return jsonList(
    rows.map((p) => ({
      ...publicPiece(
        p,
        p.entity_id === null ? null : (slugs.get(p.entity_id) ?? null),
        p.matched_entry_id === null ? null : (entrySeq.get(p.matched_entry_id) ?? null)
      ),
