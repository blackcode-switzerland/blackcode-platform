...
import { createCategory, listCategories, ManagementRefused, publicCategory } from '@/lib/db/queries/management'
...
export const GET = apiHandler(async (req: NextRequest, { params }: Params) => {
  ...
  if (!entity) {
    throw Errors.badRequest('bad_scope', slug ? `no book with slug "${slug}"` : 'no books exist in this workspace', slug ? `known books: ${entities.map((e) => e.slug).join(', ')}` : 'create one with `bk books entity create`')
  }
  const rows = await listCategories(entity.id)
  return jsonList(rows.map((c) => publicCategory(c, entity.slug)), null)
})
...
export const POST = apiHandler(async (req: NextRequest, { params }: Params) => {
  ...
  if (!body) throw Errors.badRequest('bad_json', 'the payload is not JSON', 'bk books category create')
  const entitySlug = typeof body.entity === 'string' && body.entity.trim() ? body.entity.trim() : null
  if (!entitySlug) throw Errors.badRequest('missing_field', 'entity is required', 'which book gets the category?')
  ...
  try {
    ...
    return NextResponse.json(publicCategory(row, entity?.slug ?? entitySlug), { status: 201 })
  } catch (e) {
